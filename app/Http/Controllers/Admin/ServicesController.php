...
class ServicesController extends Controller
{
    ...
    public function store(StoreServiceRequest $request)
    {
        ...
        if ($request->input('photo_2', false)) {
            $service->addMedia(storage_path('tmp/uploads/' . basename($request->input('photo_2'))))->toMediaCollection('photo_2');
        }
    ...
    public function update(UpdateServiceRequest $request, Service $service)
    {
        ...
        if ($request->input('photo_2', false)) {
            if (!$service->photo_2 || $request->input('photo_2') !== $service->photo_2->file_name) {
                if ($service->photo_2) {
                    $service->photo_2->delete();
                }
                $service->addMedia(storage_path('tmp/uploads/' . basename($request->input('photo_2'))))->toMediaCollection('photo_2');
            }
        } elseif ($service->photo_2) {
            $service->photo_2->delete();
        }
